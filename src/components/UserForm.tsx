      ...
      <div className="bg-slate-900 rounded-3xl border border-white/10 w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="flex items-center justify-between p-6 md:p-8 border-b border-white/10">
          <div>
            ...
            <p className="text-purple-400/60 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Access Control Management</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-purple-500/50 transition-all font-light"
                  placeholder="user@hatton-garden.com"
                />
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-[10px] text-gray-500 font-bold tracking-widest uppercase mb-2 block">Administrative Role</label>
                  <select
                    value={formData.role || ''}
                  ...
                  >
                    ...
                    <option value="Editor" className="bg-slate-800">Content Editor</option>
                    <option value="Moderator" className="bg-slate-800">Moderator</option>
                  </select>
                </div>

                <div>
                  <label className="text-[10px] text-gray-500 font-bold tracking-widest uppercase mb-2 block">Account Status</label>
                  <select
          ...
          <div className="flex-none flex flex-col md:flex-row items-center justify-end gap-3 p-4 md:p-8 border-t border-white/10 bg-slate-900/50 backdrop-blur-md">
            ...
            >
              {loading ? 'Updating...' : mode === 'create' ? 'Create Account' : 'Save Profile'}
            </button>
          </div>
        </form>
      </div>
